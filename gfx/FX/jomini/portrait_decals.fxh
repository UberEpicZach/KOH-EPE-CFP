...
PixelShader =
{
	TextureSampler DecalDiffuseArray
	{
		...
		MinFilter = "Linear"
		MipFilter = "Linear"
		SampleModeU = "Wrap" #EK2
		SampleModeV = "Wrap" #EK2
		type = "2darray"
	}
	...
	[[		
		...
		bool AlmostEquals(float3 Sample, float3 Mask)
		{
			...
			float3 MaskTolerance = (Mask * 0.96f) + 0.02f;

			if (
			Sample.r >= min(MaskTolerance.r,Mask.r) &&
			Sample.r <= max(MaskTolerance.r,Mask.r) &&
			Sample.g >= min(MaskTolerance.g,Mask.g) &&
			Sample.g <= max(MaskTolerance.g,Mask.g) &&
			Sample.b >= min(MaskTolerance.b,Mask.b) &&
			Sample.b <= max(MaskTolerance.b,Mask.b))
			{			
				return true;
			}
			else
...
/////////////////////////			DECODE MIPS			///////////////////////////////
	//////////////////////////////////////////////////////////////////////////////
		...
		void DecodeDiffuseMIPs( uint InstanceIndex, inout float4 Sample, float2 UV , inout DecalData Data, inout float2 WeightMatrix , int CustomBodyPart )
		{
...
/////////////////////////// CUSTOM COLOUR OVERRIDES //////////////////////////////////	
			...
			//R - OVERRIDE COLOUR BY SAMPLING TOP LEFT CORNER OF MIP WHICH STORES A COLOUR PALETTE, CHANGES COLOUR WITH WEIGHT OF DECAL - SETS WEIGHT TO 100%
